...
function UploadResume({snackOpen}:Props) {
  ...
  async function handleSubmit(event:FormEvent<HTMLFormElement>){
    ...
    if(Boolean(result.success)){
      ...
      snackOpen(false,'Resume update failed !')
    }
    setFile(null);
    setSubmit(false);
  }
  ...
  return (
    <div>
        <form onSubmit={handleSubmit}>
            <label className={classes.custum_file_upload} htmlFor="resume">
            ...
            <input className={classes.file_input} name='resume' type="file" id="resume" accept="application/pdf" onChange={handleFile}/>
            {
              file && (
